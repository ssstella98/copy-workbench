...
为迪士尼 Digital Producer 团队构建一个**文案管理工作台（Copy Workbench）**，覆盖从 Figma 设计稿 → Key 生成 → 文案撰写的全流程。前端原型为单文件 HTML（`figma-key-demo.html`），目前约 2,500+ 行。

### 两大核心模块

| 模块 | 状态 | 功能 |
|------|------|------|
| Key Generator | 稳定 | Figma API → 扫描 TEXT 节点 → 自动生成 `Domain.Page.Module.Title.Body` Key → 编辑/隐藏/导出 |
| Copywriting | 已完成 | 三层 Prompt 架构 → AI 写文案 → 合规审查 → 保存 |
...
- 字段隐藏/删除、Frame 间翻页（键盘 ← →，弹窗打开时自动禁用）
- 导出 JSON / CSV / 视觉文档（左图右表，30:70 比例，CSS 叠加定位）
...
- 三层 Prompt 架构：
  ...
  - Layer 3: 字段级提示（每行 🤖 按钮打开弹窗填写，可选）
- 文案编辑表格：`# | Key Name | 💬 | 🇨🇳中文 | 🇺🇸English | ✓合规 | 🤖`
- **单条 AI 生成**：每行 🤖 → 弹窗 → 填提示 → 🪄 生成 → 预览中/英 + 合规检查 → ✅ 填入
  - 确认填入后自动取消「仅显示未完成」勾选，立即显示结果
  - 自动保存到 IndexedDB
- **批量 AI 生成**：页面维度，收集当前 Frame 全部未完成字段，逐条调混元 API
  - 超过 50 个字段弹确认框
  - 工具栏实时进度（每 5 条更新）
  - 完成后自动取消筛选 + 自动保存
...
1. **补充 `key-rules-config.json` 的翻译词表**：目前 fallback 词表 120+ 条，HMH 新词持续补充
2. **视觉文档导出优化**：测试打印/PDF 效果，调整表格列宽适配更多 Key 长度

### 中期

3. **后端开发**：Figma API 代理（解决 Token 安全性 + 限流）、用户认证、数据库
4. **AI 翻译优化**：Copywriting Tab 中 EN 字段的 AI 翻译质量和速度
5. **多 Figma 文件支持**：项目关联多个 Figma 文件（数据结构已是数组，改 UI 即可）
6. **混元模型调优**：根据生产反馈调整 prompt 和参数
...
8. **混元 API 格式** → 用的是 OpenAI 兼容格式（`/v1/chat/completions`），`Authorization: Bearer <key>`（key 以 `sk-` 开头），响应在 `data.choices[0].message.content`。前端通过 `HUNYUAN_API` 常量统一管理端点，不要硬编码 URL。
...
10. **「仅显示未完成」筛选陷阱** → 该复选框默认勾选。AI 生成/批量生成填入英文后，字段会被过滤出表格（因为已"完成"）。`confirmAICopy` 和 `batchAICopy` 在完成后会自动取消勾选。如果新增类似功能，注意同步处理。

11. **弹窗打开时禁用键盘翻页** → 已加 `document.querySelector('.modal-overlay.show')` 检测。如果新增键盘快捷键，注意判断弹窗状态。

12. **GitHub Push Protection** → 这个仓库开启了 Secret Scanning。不要提交包含 Figma Token 或 API Key 的文件。`终端存储的输出.txt` 已被 gitignore。`figma-response.json` 是缓存数据，注意不要包含 token。
